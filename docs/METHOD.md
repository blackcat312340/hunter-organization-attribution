...
### 12.3 ROR public-suffix domain quality gate (Phase 5.2)

ROR direct identity is further gated by the **Public Suffix List (PSL)**. The PSL is the formal specification of shared public suffixes (for example `com`, `co.uk`, `edu.cn`, `github.io`); a public suffix is not itself a registrable domain and must never become an organization authority key that propagates identity to every subdomain under it. Using a frozen PSL snapshot (`https://publicsuffix.org/list/public_suffix_list.dat`, SHA256 pinned in the runtime manifest), each canonical source-provided ROR domain is checked: when `canonical_domain == public_suffix(canonical_domain)` the entry is dropped **before** ambiguity detection. The matcher implements the PSL reference algorithm — normal, wildcard (`*.foo`), and exception (`!bar.foo`) rules with longest-match — over both the ICANN and PRIVATE sections. The frozen PSL identity (`psl_source`, `psl_sha256`, `psl_retrieved_at`) is recorded in the ROR authority audit. The downstream domain suffix matcher is unchanged: only the authority ingestion quality gate is corrected. See `docs/DATA_AUTHORITIES.md` for the full policy and per-gate adapter counters.
